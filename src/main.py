...
async def main_new_york_urban(df_seen_events: pd.DataFrame = None) -> tuple[list[dict], bool]:
    use_file_logging = os.environ.get("GITHUB_ACTIONS") is None
    logger = create_logger(
        nyu_config.FILEPATH_LOG.format(date=dt.date.today().strftime("%Y-%m-%d")) if use_file_logging else None,
        nyu_config.LOGGER_NAME
    )
    logger.info(f"Starting {nyu_config.ORG_DISPLAY_NAME} scraper on {nyu_config.AJAX_URL}...")
    df_seen_events = df_seen_events[df_seen_events["organization"] == nyu_config.ORG_DISPLAY_NAME]
    try:
        new_events = nyu_scraper.get_events()
        ...
        logger.info(f"{nyu_config.ORG_DISPLAY_NAME} scraper completed successfully. Found {len(new_events)} new events.")
        return new_events, True
    except Exception as e:
        logger.warning(f"Execution failed.")
        logger.exception(e)
